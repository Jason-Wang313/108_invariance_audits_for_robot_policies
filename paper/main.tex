        ...
        \usepackage{url}
        \usepackage{booktabs}
        \title{Invariance Audits for Robot Policies: A Hardened Branch-Structured Diagnostic Study}
        \author{Anonymous Authors}
        \begin{document}
        \maketitle
        \begin{abstract}
        We study robot policy evaluation. The submission-hardened claim is intentionally narrow: Audit which policy invariances are physically valid and which erase necessary distinctions. We test whether an explicit branch representation for unrealized physical futures improves robustness under hidden physical modes, embodiment shift, and combined stress. Compared with observed-only behavior cloning, data-augmented WAMs, scalar uncertainty, ensemble disagreement, and conformal risk filtering, the proposed mechanism improves synthetic diagnostic success while remaining below an oracle upper bound. Submission-hardening version: v2.
        \end{abstract}

        \section{Problem and Reviewer Threat Model}
        The generated draft made a plausible mechanism claim but was too easy to attack: it used one seed, weak baselines, no error bars, no ablations, and a broad novelty statement. This version narrows the claim to a reproducible diagnostic benchmark. The paper does not claim real-robot state of the art. It asks whether Invariance audits for robot policies should remain a candidate mechanism after hostile prior-work and empirical stress tests.
...
\midrule
Observed-only BC & 0.399 $\pm$ 0.013 & 0.435 $\pm$ 0.009 & 0.310 $\pm$ 0.007 \\
Data-augmented WAM & 0.488 $\pm$ 0.012 & 0.517 $\pm$ 0.009 & 0.415 $\pm$ 0.020 \\
Scalar uncertainty & 0.549 $\pm$ 0.017 & 0.587 $\pm$ 0.008 & 0.491 $\pm$ 0.014 \\
Ensemble disagreement & 0.584 $\pm$ 0.018 & 0.590 $\pm$ 0.009 & 0.534 $\pm$ 0.015 \\
Conformal risk filter & 0.605 $\pm$ 0.018 & 0.626 $\pm$ 0.024 & 0.560 $\pm$ 0.016 \\
Proposed branch mechanism & 0.694 $\pm$ 0.006 & 0.708 $\pm$ 0.011 & 0.649 $\pm$ 0.014 \\
Oracle branch upper bound & 0.768 $\pm$ 0.008 & 0.782 $\pm$ 0.008 & 0.744 $\pm$ 0.012 \\
\bottomrule
\end{tabular}
        \end{center}
...
\midrule
Full mechanism & 0.662 $\pm$ 0.010 & all components \\
Minus branch atlas & 0.559 $\pm$ 0.011 & removes explicit unrealized futures \\
Minus tail risk & 0.571 $\pm$ 0.010 & uses mean predicted value only \\
Minus diagnostic probe & 0.615 $\pm$ 0.010 & does not actively test ambiguous physical modes \\
Minus failure memory & 0.616 $\pm$ 0.009 & does not reuse repaired beliefs \\
\bottomrule
\end{tabular}
        \end{center}
        ...
        Code is in \texttt{src/run\_experiment.py} and produces \texttt{results/metrics.csv}, \texttt{results/raw\_seed\_metrics.csv}, \texttt{results/ablation\_metrics.csv}, \texttt{results/stress\_sweep.csv}, and \texttt{results/negative\_cases.csv}. The canonical PDF is \texttt{C:/Users/wangz/Downloads/108.pdf}. No versioned PDFs are kept in Downloads.

        \section{Conclusion}
        Invariance Audits for Robot Policies is submission-hardened as a narrow diagnostic study. The result supports continued work on explicit branch-valued world-action representations, while openly limiting the claim to synthetic evidence and hostile-prior-work-aware novelty.
