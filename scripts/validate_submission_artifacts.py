...
PAPER = ROOT / "paper"
DOWNLOADS_PDF = Path.home() / "Downloads" / "108.pdf"
DESKTOP_PDF = Path.home() / "Desktop" / "108.pdf"
FACTORY_ROOT_PDF = ROOT.parent / "108.pdf"
CHILD_ROOT_PDF = ROOT / "108.pdf"
...
def main():
    ...
    if not DOWNLOADS_PDF.exists():
        raise AssertionError("Downloads/108.pdf missing")
    if DESKTOP_PDF.exists():
        raise AssertionError("visible Desktop 108.pdf must not exist")
    if FACTORY_ROOT_PDF.exists():
        raise AssertionError("factory-root 108.pdf must not exist")
    if CHILD_ROOT_PDF.exists():
        raise AssertionError("child-root 108.pdf must not exist")
    ...
    print(f"validated Paper 108 artifacts: pages={pages}, sha256={paper_hash}")
